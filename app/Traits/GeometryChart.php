...
trait GeometryChart
{
    ...
    function getElevation($id, $lat, $long)
    {
        ...
        $ele = 0.0;
        $exists = \Illuminate\Support\Facades\File::exists("route_data/route_$id/elevations/14/$tile[0]/$tile[1].txt");
        if ($exists) {
            $string = \Illuminate\Support\Facades\File::get(public_path("route_data/route_$id/elevations/14/$tile[0]/$tile[1].txt"));
            $line = explode("\n", $string);
